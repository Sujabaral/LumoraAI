...
def style_from_user_mode(user_mode: str, detected_style: str) -> str:
    """
    Map account preferred_mode -> the style labels used by policy/templates.
    preferred_mode: auto | listener | coach | therapist | balanced
    detected_style: whatever detect_style() returns in your project
    """
    um = (user_mode or "auto").lower().strip()
    ds = (detected_style or "neutral").lower().strip()

    if um == "auto":
        return ds

    # These are your brain style labels (adjust if yours differ)
    if um == "listener":
        return "validation_seeker"   # warmer, more supportive

    if um == "coach":
        return "problem_solver"      # step-by-step, practical

    if um == "therapist":
        return "overthinker"         # reflective, deeper
