...
# Short/overwhelmed replies that mean the user can't follow multi-step instructions yet
_UNSURE_SET = {"i can't", "i cant", "cant", "can't", "idk", "i don't know", "i dont know", "not sure", "no", "can't do it"}
...
def _emotion_strategy(emotion: str, intensity: int, style: str, trigger: Optional[str]) -> Strategy:
    ...
    if e == "panic":
        return {"name": "panic_stabilize", "tone": "urgent_calm", "tool": "breathing", "focus": "stabilize"}
...
def choose_strategy(
    ...
    # 2) Overwhelm / can't-follow-steps loop breaker
    t = (user_text_en or "").strip().lower()
    if t in _UNSURE_SET:
        s = {"name": "grounding_microstep", "tone": "urgent_calm", "tool": "grounding", "goal": "stabilize"}
        return _personalize_with_profile(s, profile)
